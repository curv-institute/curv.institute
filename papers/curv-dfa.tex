...
Using sign-correct tests with positive controls and composability analysis, we show that these channels act independently and compose stably. Curv-DFA does not propose a universal functional; it provides a modular control architecture, validated in-sample in controlled numerical experiments on small model systems (H$_2$, H$_2^+$, CO, NO; def2-svp/PBE), for diagnosing and regulating representational failure within density functional approximations.
\end{abstract}
...
\begin{equation}
J_{\mathrm{eff}}[\rho] = J[\rho] - \lambda_{\mathrm{SCC}}\,J[w_{\mathrm{eff}}\rho],
\end{equation}
with $\lambda_{\mathrm{SCC}}$ controlling the strength of regulation. $\lambda_{\mathrm{SCC}}$ is a dimensionless implementation parameter of the SCC channel; it has no derived relation to any variational multiplier in the broader Representational Science framework. SCC is implemented using a frozen-gate approximation, sufficient to establish sign correctness, stability, and composability.
...
When geometry curvature control and SCC are applied simultaneously, deviations from additivity remain below 2\% across all test systems (re-verified with the current SCC v2 implementation on 2026-06-09: maximum relative composability error 1.38\% on H$_2$ and 0.99\% on H$_2^+$; see \texttt{curv-dfa/COMPOSABILITY\_SCCV2.md}), demonstrating orthogonality and composability of the control channels.
